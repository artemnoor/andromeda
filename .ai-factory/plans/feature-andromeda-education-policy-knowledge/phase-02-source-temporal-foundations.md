...
- Temporal decisions: UTC-normalize aware instants; valid-time intervals are half-open `[start, end)`; admission application/enrollment periods are inclusive civil dates. Immutable revisions use `recorded_at` as their system-time start, and as-known-at reads select the latest recorded revision no later than the requested instant. A missing valid interval is unknown (`None`), never unbounded applicability. Admission and academic years remain independent source-backed dimensions.
- Шаги: add typed temporal contracts and admissions-owned cycle revisions with field-level `EvidenceRef`; provide deterministic as-known-at reads; persist additively; return `BLOCKED_BY_MISSING_DATA` when no reviewed cycle exists; document unavailable history without inventing dates.
