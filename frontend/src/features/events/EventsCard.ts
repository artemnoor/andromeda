...
export function renderEventCard(event: Event, programs: readonly Program[]): string {
  ...
  return `<article class="event-card" data-testid="event-card" data-event-id="${escapeHtml(event.id)}"><div class="event-card-meta"><span class="status">${escapeHtml(kindLabels[event.kind])}</span><span class="status">${escapeHtml(formatLabels[event.format])}</span></div><h3>${escapeHtml(event.title)}</h3><time data-testid="event-date" datetime="${escapeHtml(event.startsAt)}">${escapeHtml(date)}</time>${event.description ? `<p class="event-description">${escapeHtml(event.description)}</p>` : ""}<div class="event-card-details">${links}${venue}</div><div class="event-card-actions">${registration}<code>${escapeHtml(event.id)}</code></div></article>`;
}
