...
function renderEventStep(step: Step, programs: readonly Program[]): string {
  ...
  return `<article class="personal-route-step" data-testid="personal-route-step" data-step-kind="${escapeHtml(step.kind)}"><div class="personal-route-step-head"><span class="eyebrow">${String(step.position).padStart(2, "0")} · Посетить событие</span><span class="status">${escapeHtml(event.format)}</span></div><h3>${escapeHtml(event.title)}</h3><time datetime="${escapeHtml(event.startsAt)}">${escapeHtml(formatDate(event.startsAt))}</time>${event.description ? `<p>${escapeHtml(event.description)}</p>` : ""}<div class="personal-route-event-details"><div><span class="label">Связанные программы</span><p>${escapeHtml(programLabels(step.programIds, programs))}</p><code>${escapeHtml(event.id)}</code></div>${point}</div><div class="personal-route-step-actions">${registration}</div></article>`;
}
