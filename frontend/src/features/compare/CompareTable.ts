...
export function renderCompareTable(response: CompareResponse): string {
  const rows = response.rows.map((row) => {
    ...
    return `<tr class="${tone}"><td>${row.semester ?? "—"}</td><th scope="row">${escapeHtml(row.discipline.name)}<small>${escapeHtml(row.subjectGroup ?? "Без блока")}</small><small class="area-hint">${areas}</small></th>${workloadCell(row.a)}${workloadCell(row.b)}<td><strong>${row.hoursDelta ?? "—"} ч</strong><br><span>${row.creditsDelta ?? "—"} ЗЕТ</span></td><td><span class="status">${escapeHtml(row.status)}</span></td></tr>`;
  }).join("");
