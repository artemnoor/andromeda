...
export function renderAppShell(root: HTMLElement, programs: readonly Program[]): void {
  ...
  if (!featureRoot || !compareButton || !eventsButton || !personalRouteButton || !proftestButton || !programButton) return;

  type Feature = "compare" | "events" | "personal-route" | "proftest" | "program";
  ...
  const featureFromHash = (): { feature: Feature; programId?: string } => {
    ...
    if (window.location.hash === "#personal-route") return { feature: "personal-route" };
