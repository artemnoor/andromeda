...
PRODUCTION_LOCK_PATH = ROOT / "config" / "jev" / "locks" / "next-action.typesafe-jev.v2.lock.json"
PRODUCTION_MANIFEST_PATH = PRODUCTION_LOCK_PATH.with_suffix(PRODUCTION_LOCK_PATH.suffix + ".meta.json")
PRODUCTION_OBSERVATIONS_PATH = ROOT / "evals" / "jev" / "corpus" / "production-next-action-observations.v2.jsonl"
PRODUCTION_CORPUS_PATH = ROOT / "evals" / "jev" / "corpus" / "production-next-action.v2.jsonl"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if min(args.min_support, args.min_total, args.min_heldout) < 1:
    ...
    manifest_output = args.manifest_output or output.with_suffix(output.suffix + ".meta.json")
    corpus_path = args.corpus or (CORPUS_PATH if args.source == "fixture" else PRODUCTION_CORPUS_PATH)
    observations_path = args.observations or (
        OBSERVATIONS_PATH if args.source == "fixture" else PRODUCTION_OBSERVATIONS_PATH
    )
    if args.check:
        _validate_lock(output, manifest_output, observations_path=observations_path)
...
def _build_lock(
    *,
    ...
    if source_kind == "production":
        definition_ids = PRODUCTION_DEFINITION_IDS
    else:
        observed_ids = tuple(dict.fromkeys(_required_string(row, "definition_id") for row in source_rows))
        # Noul evidence is not exposed as a calibrated Choice by the current runtime adapter.
        definition_ids = tuple(item for item in observed_ids if item != "semantic-feature.v1")
    decision_rows = [row for row in all_cases if row.get("definition_id") in definition_ids]
    if not decision_rows or not source_rows:
        raise CalibrationArtifactError("calibration corpus and observations must not be empty")
    bundle = build_jevcal_bundle(registry, tuple(decision_rows), definition_ids=definition_ids)
    case_by_id = {str(row["id"]): row for row in bundle.rows}
    selected_observations = [row for row in source_rows if row.get("definition_id") in definition_ids]
    observation_ids = [_required_string(row, "case_id") for row in selected_observations]
    if len(observation_ids) != len(set(observation_ids)):
        raise CalibrationArtifactError("calibration observation case IDs must be unique")
    ...
    with tempfile.TemporaryDirectory(prefix="andromeda-jevcal-") as directory:
        ...
        questions_path.write_text(_questions_yaml(bundle.questions, model=model), encoding="utf-8")
            ...
            for observation in question_observations:
                ...
                if source_kind == "production":
                    if observation.get("model_requested") != model:
                        raise CalibrationArtifactError(f"requested model mismatch: {case_id}")
                    if observation.get("definition_version") != registry.get(question_id).version:
                        raise CalibrationArtifactError(f"question version mismatch: {case_id}")
                    expected_split = "heldout" if in_holdout(case_id, HOLDOUT, SEED) else "train"
                    if observation.get("split") != expected_split:
                        raise CalibrationArtifactError(f"upstream holdout assignment mismatch: {case_id}")
                    if observation.get("capture_method") != "official-typesafe-sdk-system-one":
                        raise CalibrationArtifactError(f"observation is not from the official SDK: {case_id}")
                    if observation.get("gold") != gold:
                        raise CalibrationArtifactError(f"observation gold differs from corpus: {case_id}")
                    ...
                    if observation.get("correct") is not correct:
                        raise CalibrationArtifactError(f"observation correctness is inconsistent: {case_id}")
                    ...
                    if observed_version == "unknown":
                        raise CalibrationArtifactError(f"model version is not identifiable: {case_id}")
                    ...
                    if not isinstance(provided_confidence, (int, float)):
                        raise CalibrationArtifactError(f"model confidence is missing: {case_id}")
                else:
                    # Fixture-only synthesis keeps this wiring artifact explicitly non-production.
                    fixture_confidence = _probability(observation)
                    prediction = gold if bool(observation.get("correct")) else next(
                        label for label in labels if label != gold
                    )
                    probabilities = _fixture_distribution(labels, prediction, fixture_confidence)
                    correct = prediction == gold
                ...
                records.append(
                    Record(
                        ...
                        gold_key=gold,
                        measures=confidence_measures(probabilities, float(provided_confidence)),
                    )
                )
            if not records:
        ...
        if source_kind == "production" and (len(model_versions) != 1 or len(providers) != 1):
            raise CalibrationArtifactError("production observations must use one stable model and provider")
        lock = build_lock(
            questions,
            rows,
            results,
            ...
            } if source_kind == "production" else None,
            holdout=HOLDOUT,
            seed=SEED,
...
def _build_manifest(
    ...
    *,
    ...
    definition_ids = sorted(str(key) for key in raw_questions) if isinstance(raw_questions, dict) else []
    ...
    for definition_id in definition_ids:
        ...
        heldout: dict[str, object] = heldout_value if isinstance(heldout_value, dict) else {}
        calibration_value = result.get("calibration")
        calibration: dict[str, object] = calibration_value if isinstance(calibration_value, dict) else {}
        ...
        if source_kind == "production":
            ...
            if result.get("status") != "ok" or not isinstance(result.get("threshold"), (int, float)):
            ...
            if not isinstance(fit_accepted, int) or fit_accepted < min_support:
                gate_reasons.append(f"{definition_id}:accepted_fit_support_below_minimum")

    observed_models = lock.get("model_observed")
    model_observed = observed_models[0] if isinstance(observed_models, list) and len(observed_models) == 1 else None
    observations = _read_jsonl(observations_path)
    providers = {str(row.get("provider")) for row in observations} if source_kind == "production" else set()
    capture_methods = {str(row.get("capture_method")) for row in observations} if source_kind == "production" else set()
    endpoint_hosts = {str(row.get("endpoint_host")) for row in observations if row.get("endpoint_host")} if source_kind == "production" else set()
    if source_kind == "production":
        if definition_ids != sorted(PRODUCTION_DEFINITION_IDS):
            gate_reasons.append("calibration_definition_scope_mismatch")
        if not isinstance(model_observed, str) or not model_observed or model_observed == "unknown":
        ...
        if len(endpoint_hosts) != 1 or not endpoint_hosts.issubset({"polza.ai", "api.typesafe.ai"}):
    ...
    return {
        ...
        "status": "fixture_only" if source_kind == "fixture" else ("production_ready" if not gate_reasons else "not_production_ready"),
        ...
        "provider": next(iter(providers)) if len(providers) == 1 else lock.get("provider"),
        "endpoint_host": next(iter(endpoint_hosts)) if len(endpoint_hosts) == 1 else None,
...
def _validate_lock(
    ...
    *,
    ...
    selected_observations = observations_path or (
        PRODUCTION_OBSERVATIONS_PATH if source_kind == "production" else OBSERVATIONS_PATH
    )
    ...
    if not isinstance(manifest_ids, list) or set(manifest_ids) != question_ids:
        raise CalibrationArtifactError("manifest calibration scope does not match upstream lock")
    if not question_ids or not question_ids.issubset(expected_ids):
        raise CalibrationArtifactError("upstream lock contains unknown or no registry definitions")
    ...
    if source_kind == "production":
        if manifest.get("status") != "production_ready":
            raise CalibrationArtifactError("production calibration gates are not satisfied")
        observed = lock.get("model_observed")
        if not isinstance(observed, list) or len(observed) != 1 or manifest.get("model_observed") != observed[0]:
            raise CalibrationArtifactError("calibration observed model version is inconsistent")
        if manifest.get("model_requested") != lock.get("model_requested"):
            raise CalibrationArtifactError("calibration requested model is inconsistent")
        if not set(PRODUCTION_DEFINITION_IDS).issubset(question_ids):
            raise CalibrationArtifactError("production lock does not calibrate next-action")
    logger.info(
...
def _questions_yaml(questions: dict[str, dict[str, object]], *, model: str) -> str:
    ...
    return yaml.safe_dump({"model": model, "questions": questions}, allow_unicode=True, sort_keys=False)
...
def _fixture_distribution(
    ...
    return {
        label: probability if label == prediction else remainder
        for label in labels
    }
