    ...
    *,
    ...
    try:
        ...
        result = TracerRunResult(
            ...
            unique_plan_count=len({str(snapshot.requested_url) for snapshot in raw.snapshots if snapshot.source_kind == "bmstu_curriculum_document"}),
...
def _order_counts(raw: RawTracerBundle) -> tuple[int, int, int, int, int, int]:
    ...
    manifest_count = 0
    entries_by_url: dict[str, object] = {}
