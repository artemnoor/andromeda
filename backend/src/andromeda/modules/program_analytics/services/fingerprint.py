...
from andromeda.modules.analytics.contracts.activity import ACTIVITY_SIGNAL_WEIGHTS as _ACTIVITY_SIGNAL_WEIGHTS
...
from ..contracts.fingerprint import ActivityCode, CurriculumEvidence, DistinctiveSubject, ProgramFingerprint

logger = logging.getLogger("andromeda.program_analytics.fingerprint")
ZERO = Decimal("0")
ONE = Decimal("1")
ACTIVITY_SIGNAL_WEIGHTS = {
    area: {ActivityCode(signal.value): weight for signal, weight in values.items()}
    for area, values in _ACTIVITY_SIGNAL_WEIGHTS.items()
}
...
class FingerprintBuilder:
    ...
    def build(self, program: Program, curriculum: Curriculum, disciplines: Mapping[DisciplineId, Discipline]) -> ProgramFingerprint:
        ...
        for item in curriculum.items:
            ...
            if sum((weight.weight for weight in area_weights), ZERO) != ONE:
    ...
    def add_distinctive_subjects(self, fingerprints: Iterable[ProgramFingerprint], *, limit: int = 8) -> tuple[ProgramFingerprint, ...]:
        ...
        for fingerprint in fingerprint_list:
            ...
            for normalized_name, evidence_rows in by_name.items():
                ...
                distinctiveness = max(ZERO, min(ONE, share * rarity))
                ...
                subjects.append(
                    DistinctiveSubject(
                        ...
                        workload=workload,
                        share=_quantize_ratio(share),
                        rarity=_quantize_ratio(rarity),
                        distinctiveness=_quantize_ratio(distinctiveness),
                    )
                )
...
def _shares(values: Mapping[KeyT, Decimal], total: Decimal) -> dict[KeyT, Decimal]:
    ...
    if not ordered_values:
        return {}
    shares = {key: _quantize_ratio(value / total) for key, value in ordered_values}
    last_key = ordered_values[-1][0]
    shares[last_key] += ONE - sum(shares.values(), ZERO)
    return shares
...
def _activity_signals(area_share: Mapping[DisciplineAreaCode, Decimal]) -> dict[ActivityCode, Decimal]:
    ...
    normalized = {signal: _quantize_ratio(value / total) for signal, value in ordered_signals}
    last_signal = ordered_signals[-1][0]
    normalized[last_signal] += ONE - sum(normalized.values(), ZERO)
    return normalized
