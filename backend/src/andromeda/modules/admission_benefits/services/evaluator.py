...
class AdmissionBenefitEvaluator:
    ...
    def evaluate(self, request: AdmissionBenefitEvaluationInput) -> AdmissionEligibilityResult:
        ...
        eligible = tuple(item for item in evaluations if item.status is EligibilityStatus.ELIGIBLE)
        review = tuple(item for item in evaluations if item.status is EligibilityStatus.REVIEW_REQUIRED)
        status = (
    ...
    def _evaluate_rule(
        ...
        if rule.status is not RuleDataStatus.ACTIVE:
            return self._evaluation(rule, EligibilityStatus.REVIEW_REQUIRED, "Rule is not active", evidence), ("rule status requires review",)
        if rule.admission_year != request.admission_year:
            return (
                self._evaluation(rule, EligibilityStatus.REVIEW_REQUIRED, "Rule admission year does not match request", evidence),
                ("rule admission year mismatch",),
            )
        if rule.education_level is not None and request.education_level is not rule.education_level:
            return self._evaluation(rule, EligibilityStatus.NOT_ELIGIBLE, "Rule education level does not match request", evidence), ()
        if rule.route in {AdmissionRoute.OLYMPIAD, AdmissionRoute.VOSH, AdmissionRoute.INTERNATIONAL}:
            applicant_fact = next(
                (
                    fact
                    ...
                    and (rule.olympiad_profile_id is None or fact.olympiad_profile_id == rule.olympiad_profile_id)
                ),
                None,
            )
            if applicant_fact is None:
                return self._evaluation(rule, EligibilityStatus.NOT_ELIGIBLE, "Applicant has no matching Olympiad result", evidence), ()
            ...
            if validity.status is not EligibilityStatus.ELIGIBLE:
                return self._evaluation(rule, validity.status, validity.reason, evidence, matched_fact), _gap(validity.status, validity.reason)
            ...
            if scope.status.value != "applicable":
                ...
                return self._evaluation(rule, status, scope.reason, evidence, matched_fact), _gap(status, scope.reason)
            ...
            if confirmation.status is not EligibilityStatus.ELIGIBLE:
                return self._evaluation(rule, confirmation.status, confirmation.reason, evidence, matched_fact), _gap(confirmation.status, confirmation.reason)
            change = None
            if rule.benefit_type is BenefitType.ONE_HUNDRED_POINTS:
                current = next(
                    (score.score for score in request.applicant.ege_scores if _same_subject(score.subject, rule.target_subject or "")),
                    None,
                )
                change = Decimal(100) - current if current is not None else None
            return self._evaluation(rule, EligibilityStatus.ELIGIBLE, "All source-backed conditions passed", evidence, matched_fact, change), ()
        return self._evaluation(rule, EligibilityStatus.REVIEW_REQUIRED, "Non-Olympiad route requires a dedicated source policy", evidence), ("route policy requires review",)
    ...
    def _evaluation(
        ...
        return AdmissionBenefitEvaluation(
            ...
            rejection_reason=None if status is EligibilityStatus.ELIGIBLE else reason[:256],
            effective_score_change=score_change,
            points_contribution=Decimal(100) if status is EligibilityStatus.ELIGIBLE and rule.benefit_type is BenefitType.ONE_HUNDRED_POINTS else None,
            evidence=evidence,
        )
...
def _route_for(evaluations: tuple[AdmissionBenefitEvaluation, ...]) -> AdmissionRoute | None:
...
def _same_subject(left: str, right: str) -> bool:
    return left.casefold().replace("ё", "е").strip() == right.casefold().replace("ё", "е").strip()


def _gap(status: EligibilityStatus, reason: str) -> tuple[str, ...]:
    return (reason,) if status in {EligibilityStatus.REVIEW_REQUIRED, EligibilityStatus.INSUFFICIENT_DATA} else ()
