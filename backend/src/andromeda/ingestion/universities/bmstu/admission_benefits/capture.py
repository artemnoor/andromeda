...
class BmstuAdmissionCaptureReport(ContractModel):
    ...
    def complete(self) -> bool:
        return self.failed == 0 and self.unresolved == 0 and self.discovered == self.fetched
...
class BmstuAdmissionBenefitsCapture:
        ...
        *,
        ...
        if mode == "fixture":
            ...
            raise ContractError(
                ...
                (ErrorDetail(path="mode", message="unsupported capture mode", type="value_error"),),
            )
        logger.info(
    ...
    def _capture_live(self, admission_year: int) -> BmstuAdmissionCaptureResult:
        ...
        if not resource.ok:
            gap = source_fetch_gap(INDEX_SOURCE_KIND, BMSTU_ADMISSION_DOCUMENTS_INDEX_URL, resource.error_code or "source_unavailable")
            manifest = BmstuAdmissionSourceManifest(index_url=cast(HttpUrl, BMSTU_ADMISSION_DOCUMENTS_INDEX_URL), admission_year=admission_year)
            return self._result(manifest, (), (gap,), failed=1, diagnostics=("index_fetch_failed",))
        index_snapshot = self._resource_snapshot(INDEX_SOURCE_KIND, BMSTU_ADMISSION_DOCUMENTS_INDEX_URL, resource)
        try:
            manifest = discover_document_manifest(resource.body, admission_year=admission_year)
        except ContractError:
            gap = source_fetch_gap(INDEX_SOURCE_KIND, BMSTU_ADMISSION_DOCUMENTS_INDEX_URL, "index_parse_failed")
            empty = BmstuAdmissionSourceManifest(index_url=cast(HttpUrl, BMSTU_ADMISSION_DOCUMENTS_INDEX_URL), admission_year=admission_year)
            return self._result(empty, (index_snapshot,), (gap,), failed=1, diagnostics=("index_parse_failed",))
        return self._append_profile_sources(self._fetch_selected(manifest, (index_snapshot,)))

    def _capture_fixture(self, fixture_dir: Path | None, admission_year: int) -> BmstuAdmissionCaptureResult:
        if fixture_dir is None:
            raise ContractError(ErrorCode.SOURCE_CONTRACT_ERROR, "BMSTU admission benefits fixture directory is required")
        ...
        if not index_path.exists():
            raise ContractError(
                ...
                (ErrorDetail(path="fixture_dir", message=str(index_path), type="source_fixture"),),
            )
        index_body = index_path.read_bytes()
        manifest = discover_document_manifest(index_body, admission_year=admission_year)
        index_snapshot = _fixture_snapshot(INDEX_SOURCE_KIND, BMSTU_ADMISSION_DOCUMENTS_INDEX_URL, index_body)
        ...
        for document in manifest.selected:
            candidate = _fixture_document_path(fixture_dir, document.index_id, document.kind)
            if candidate is None:
                gaps.append(source_fetch_gap(f"bmstu_admission_document:{document.kind.value}", str(document.url), "document_fixture_missing"))
                diagnostics.append(f"fixture_missing:{document.kind.value}")
                continue
            body = candidate.read_bytes()
            if not body:
                gaps.append(source_fetch_gap(f"bmstu_admission_document:{document.kind.value}", str(document.url), "document_fixture_empty"))
                diagnostics.append(f"fixture_empty:{document.kind.value}")
                continue
            snapshots.append(_fixture_snapshot(_document_source_kind(document.kind), str(document.url), body))
            fetched += 1
        return self._append_profile_sources(self._result(
            manifest,
            tuple(snapshots),
            tuple(gaps),
            fetched=fetched,
    ...
    def _append_profile_sources(
        ...
        *,
        ...
        for profile_key, profile_url in BMSTU_OFFICIAL_OLYMPIAD_PROFILE_SOURCES:
            if any(snapshot.source_kind == f"bmstu_olympiad_profile:{profile_key}" for snapshot in snapshots):
                continue
            if fixture_dir is not None:
                candidate = fixture_dir / f"shag-{profile_key}.html.extract.json"
                if candidate.exists():
                    snapshots.append(_fixture_profile_snapshot(candidate, profile_url, profile_key))
                else:
                    gaps.append(source_fetch_gap(f"bmstu_olympiad_profile:{profile_key}", profile_url, "profile_fixture_missing"))
                continue
            resource = self.fetcher.fetch_http(profile_url)
            if not resource.ok:
                gaps.append(source_fetch_gap(f"bmstu_olympiad_profile:{profile_key}", profile_url, resource.error_code or "source_unavailable"))
                continue
            if not _is_markup(resource):
                gaps.append(source_fetch_gap(f"bmstu_olympiad_profile:{profile_key}", profile_url, "unsupported_document_body"))
                continue
            snapshots.append(self._resource_snapshot(f"bmstu_olympiad_profile:{profile_key}", profile_url, resource))
        return result.model_copy(
            update={"captured": CapturedSources(tuple(snapshots), source_gaps=tuple(gaps))}
        )
    ...
    def _fetch_selected(
        ...
        for document in manifest.selected:
            ...
            if not resource.ok:
                ...
                gaps.append(source_fetch_gap(_document_source_kind(document.kind), str(document.url), reason))
                diagnostics.append(f"fetch_failed:{document.kind.value}:{reason}")
                continue
            if not is_pdf(resource.body, resource.content_type, str(document.url)) and not _is_markup(resource):
                gaps.append(source_fetch_gap(_document_source_kind(document.kind), str(document.url), "unsupported_document_body"))
                diagnostics.append(f"unsupported_body:{document.kind.value}")
                continue
            snapshots.append(self._resource_snapshot(_document_source_kind(document.kind), str(document.url), resource))
            fetched += 1
        return self._result(manifest, tuple(snapshots), tuple(gaps), fetched=fetched, diagnostics=tuple(diagnostics))

    @staticmethod
    def _resource_snapshot(kind: str, requested_url: str, resource: FetchedResource) -> RawSourceSnapshot:
        if not resource.ok or resource.status_code is None:
            raise ContractError(ErrorCode.SOURCE_CONTRACT_ERROR, "cannot snapshot an unsuccessful resource")
    ...
    def _result(
        ...
        *,
        ...
        report = BmstuAdmissionCaptureReport(
            ...
            source_hashes=tuple(snapshot.content_sha256 for snapshot in unique_snapshots),
...
def _fixture_profile_snapshot(path: Path, requested_url: str, profile_key: str) -> RawSourceSnapshot:
    body = path.read_bytes()
    try:
        payload = json.loads(body.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError):
        payload = {}
    source_url = payload.get("source_url", requested_url) if isinstance(payload, dict) else requested_url
    ...
    return RawSourceSnapshot(
        ...
        captured_at=FIXTURE_CAPTURED_AT,
        content_sha256=source_hash if isinstance(source_hash, str) else sha256(body).hexdigest(),
        body=body,
        response_class="success",
        access_mode="fixture",
    )
...
def _deduplicate_snapshots(snapshots: tuple[RawSourceSnapshot, ...]) -> tuple[RawSourceSnapshot, ...]:
    seen: set[tuple[str, str, str]] = set()
    result: list[RawSourceSnapshot] = []
    for snapshot in snapshots:
        key = (snapshot.source_kind, str(snapshot.requested_url), snapshot.content_sha256)
        if key in seen:
            continue
        seen.add(key)
