...
_NON_STALE_STATUSES = tuple(status.value for status in RuleDataStatus if status is not RuleDataStatus.STALE)
...
class SqlAlchemyAdmissionBenefitsRepository(AdmissionBenefitRepository):
    ...
    def sync_snapshot(
        ...
        *,
        ...
        logger.info(
            ...
            len(snapshot.individual_achievement_policy.rules) if snapshot.individual_achievement_policy else 0,
            stale_rows,
            conflict_rows,
            _elapsed_ms(started),
        )
        return AdmissionBenefitSyncStats(
            ...
            unchanged_rows=olympiad_counts[1] + profile_counts[1] + rule_counts[1] + achievement_counts[1],
    ...
    def get_catalog(
        ...
        policy_model = self._policy_model(university_id, admission_year, education_level, include_review=True)
        profile_ids = {row.olympiad_profile_id for row in rules if row.olympiad_profile_id is not None}
        ...
        olympiads = self._olympiad_models(admission_year, olympiad_ids | {row.olympiad_id for row in profiles})
        if not rules and policy_model is None and not olympiads:
            return None
        policy = _to_policy(
            policy_model,
            self._achievement_rule_models(policy_model.id, policy_model.source_snapshot_hash) if policy_model else (),
        ) if policy_model else None
        source_rows: list[Any] = [*olympiads, *profiles, *rules]
        if policy_model is not None:
            source_rows.append(policy_model)
            source_rows.extend(
                self._achievement_rule_models(policy_model.id, policy_model.source_snapshot_hash)
            )
        sources = _deduplicate_sources(source_rows)
        coverage = _coverage_from_rows(source_rows, sources)
        ...
        return AdmissionBenefitsSnapshot(
            ...
            olympiad_profiles=tuple(
                self._to_profile(row, subjects=profile_subjects.get(_profile_identity(row), ()))
                for row in profiles
            ),
            benefit_rules=tuple(
                self._to_rule(
    ...
    def get_rules_for_program(
        ...
        *,
        ...
        if program is None:
            logger.info("admission_benefits_program_not_found program_id=%s year=%s", program_id, admission_year)
            return ()
        direction = self._session.get(DirectionModel, program.direction_id)
        if direction is None:
            logger.warning("admission_benefits_program_direction_missing program_id=%s", resolved_program_id)
            return ()
        ...
        logger.debug("admission_benefits_program_read program_id=%s year=%s rules=%d", resolved_program_id, admission_year, len(result))
        return result
    ...
    def get_olympiad_catalog(
        ...
        rows = self._rule_models(university_id=university_id, admission_year=admission_year, include_review=False)
        ids = {row.olympiad_id for row in rows if row.olympiad_id is not None}
        return tuple(_to_olympiad(row) for row in self._olympiad_models(admission_year, ids))
    ...
    def get_individual_achievement_policy(
        ...
        *,
        ...
        model = self._policy_model(university_id, admission_year, education_level, include_review=include_review)
        if model is None:
            return None
        return _to_policy(
            model,
            self._achievement_rule_models(model.id, model.source_snapshot_hash, include_review=include_review),
        )
    ...
    def _rule_models(
        ...
        *,
        ...
        if not include_review:
            ...
            query = query.where(AdmissionBenefitRuleModel.status.in_(_NON_STALE_STATUSES))
        ...
        return tuple(
            self._session.scalars(query.order_by(AdmissionBenefitRuleModel.id, AdmissionBenefitRuleModel.admission_year)).all()
        )
    ...
    def _policy_model(
        ...
        *,
        ...
        if education_level is None:
            query = query.where(IndividualAchievementPolicyModel.education_level == "unknown")
        else:
            query = query.where(
                IndividualAchievementPolicyModel.education_level.in_((education_level.value, "unknown"))
            )
        if not include_review:
            query = query.where(IndividualAchievementPolicyModel.status == RuleDataStatus.ACTIVE.value)
        else:
            query = query.where(IndividualAchievementPolicyModel.status.in_(_NON_STALE_STATUSES))
        return self._session.scalar(query.order_by(IndividualAchievementPolicyModel.education_level.desc()))
    ...
    def _achievement_rule_models(
        ...
        *,
        ...
        query = select(IndividualAchievementRuleModel).where(IndividualAchievementRuleModel.policy_id == policy_id)
        if policy_source_snapshot_hash is not None:
            query = query.where(
                IndividualAchievementRuleModel.policy_source_snapshot_hash == policy_source_snapshot_hash
            )
        if not include_review:
            query = query.where(IndividualAchievementRuleModel.status == RuleDataStatus.ACTIVE.value)
        else:
            query = query.where(IndividualAchievementRuleModel.status.in_(_NON_STALE_STATUSES))
        return tuple(
            self._session.scalars(query.order_by(IndividualAchievementRuleModel.id, IndividualAchievementRuleModel.admission_year)).all()
        )

    def _profile_models(self, admission_year: int, ids: Iterable[str | None]) -> tuple[AdmissionBenefitOlympiadProfileModel, ...]:
        ...
        return tuple(
            self._session.scalars(
                ...
                .where(
                    AdmissionBenefitOlympiadProfileModel.admission_year == admission_year,
                    AdmissionBenefitOlympiadProfileModel.id.in_(profile_ids),
                    AdmissionBenefitOlympiadProfileModel.status.in_(_NON_STALE_STATUSES),
                )
    ...
    def _profile_subject_rows(
        ...
        values = self._session.scalars(
            ...
            .where(
                ...
                AdmissionBenefitProfileSubjectModel.admission_year.in_(tuple(row.admission_year for row in rows)),
            )
            .order_by(
        ...
        by_identity: dict[tuple[str, int, str], list[AdmissionBenefitProfileSubjectModel]] = {}
        for value in values:
            by_identity.setdefault(
                (value.profile_id, value.admission_year, value.profile_source_snapshot_hash),
                [],
            ).append(value)
        return {key: tuple(items) for key, items in by_identity.items()}

    def _olympiad_models(self, admission_year: int, ids: Iterable[str | None]) -> tuple[AdmissionBenefitOlympiadModel, ...]:
        ...
        *,
            ...
            scope_rows = tuple(
                self._session.scalars(
                    ...
                    .where(
                        ...
                        AdmissionBenefitRuleScopeModel.admission_year == row.admission_year,
                        AdmissionBenefitRuleScopeModel.rule_source_snapshot_hash == row.source_snapshot_hash,
                    )
        ...
        try:
            scope = BenefitScope(
                ...
                targets=targets,
                excluded_targets=excluded if row.scope_mode == BenefitScopeMode.ALL.value else (),
                original_text=row.scope_original_text,
            )
        except ValueError:
            logger.warning("admission_benefits_scope_invalid rule_id=%s year=%s", row.id, row.admission_year)
            return False
        return scope.applies_to(
            direction_code=direction_code,
            program_id=program_id,
            education_level=education_level,
        ).status.value == "applicable"
    ...
    def _rule_scope_rows(
        ...
        values = self._session.scalars(
            select(AdmissionBenefitRuleScopeModel)
            .where(
                AdmissionBenefitRuleScopeModel.rule_id.in_(tuple(row.id for row in rows)),
                AdmissionBenefitRuleScopeModel.admission_year.in_(tuple(row.admission_year for row in rows)),
            )
            .order_by(
        ...
        by_identity: dict[tuple[str, int, str], list[AdmissionBenefitRuleScopeModel]] = {}
        for value in values:
            by_identity.setdefault(
    ...
    def _rule_subject_rows(
        ...
        values = self._session.scalars(
            ...
            .where(
                AdmissionBenefitRuleSubjectModel.rule_id.in_(tuple(row.id for row in rows)),
                AdmissionBenefitRuleSubjectModel.admission_year.in_(tuple(row.admission_year for row in rows)),
            )
            .order_by(
        ...
        by_identity: dict[tuple[str, int, str], list[AdmissionBenefitRuleSubjectModel]] = {}
        for value in values:
            by_identity.setdefault(
                (value.rule_id, value.admission_year, value.rule_source_snapshot_hash),
                [],
    ...
    def _to_rule(
        ...
        *,
            ...
            subjects = tuple(
                self._session.scalars(
                    ...
                    .where(
                        ...
                        AdmissionBenefitRuleSubjectModel.admission_year == row.admission_year,
                        AdmissionBenefitRuleSubjectModel.rule_source_snapshot_hash == row.source_snapshot_hash,
                    )
        ...
        return AdmissionBenefitRule(
            ...
            result_type=OlympiadResultType(row.result_type) if row.result_type else None,
            scope=BenefitScope(
                mode=BenefitScopeMode(row.scope_mode),
                targets=targets,
            ...
            points=row.points,
            conditions=tuple(BenefitCondition.model_validate(item, strict=False) for item in _json_list(row.conditions_json)),
        ...
        *,
            ...
            subjects = tuple(
                self._session.scalars(
                    ...
                    .where(
                        ...
                        AdmissionBenefitProfileSubjectModel.admission_year == row.admission_year,
                        AdmissionBenefitProfileSubjectModel.profile_source_snapshot_hash == row.source_snapshot_hash,
                    )
        ...
        return OlympiadProfile(
            ...
            corresponding_subjects=tuple(
                OlympiadProfileSubject(subject=item.subject, source_text=item.source_text) for item in subjects
            ),
            admission_year=row.admission_year,
            provenance=(_provenance(row),),
        )
    ...
    def _upsert_olympiad(self, value: Olympiad) -> str:
        ...
        return _apply_values(row, self._session, AdmissionBenefitOlympiadModel, values, identity)
    ...
    def _upsert_profile(self, value: OlympiadProfile) -> str:
        ...
        return _apply_values(row, self._session, AdmissionBenefitOlympiadProfileModel, values, identity)

    def _upsert_benefit_rule(self, value: AdmissionBenefitRule) -> str:
        identity = (value.id, value.admission_year, value.provenance.source_snapshot_hash)
        row = self._session.get(AdmissionBenefitRuleModel, identity)
        values = {
            "id": value.id,
            "admission_year": value.admission_year,
            "university_id": value.university_id,
            "education_level": value.education_level.value if value.education_level else None,
            ...
            "points": value.points,
            "conditions_json": [item.model_dump(mode="json") for item in value.conditions],
        ...
        return _apply_values(row, self._session, AdmissionBenefitRuleModel, values, identity)

    def _upsert_achievement_policy(self, value: IndividualAchievementPolicy) -> tuple[str, str]:
        ...
        _apply_values(row, self._session, IndividualAchievementPolicyModel, values, identity)
        return identity

    def _upsert_achievement_rule(self, policy: IndividualAchievementPolicy, value: IndividualAchievementRule) -> str:
        identity = (value.id, value.admission_year, value.provenance.source_snapshot_hash)
        ...
        return _apply_values(row, self._session, IndividualAchievementRuleModel, values, identity)
    ...
    def _replace_profile_subjects(self, value: OlympiadProfile) -> None:
        self._session.execute(
            delete(AdmissionBenefitProfileSubjectModel).where(
                ...
                AdmissionBenefitProfileSubjectModel.admission_year == value.admission_year,
                AdmissionBenefitProfileSubjectModel.profile_source_snapshot_hash == value.provenance[0].source_snapshot_hash,
            )
        )
    ...
    def _replace_rule_children(self, value: AdmissionBenefitRule) -> None:
        self._session.execute(
            delete(AdmissionBenefitRuleScopeModel).where(
                ...
                AdmissionBenefitRuleScopeModel.rule_source_snapshot_hash == value.provenance.source_snapshot_hash,
            )
        )
        self._session.execute(
            delete(AdmissionBenefitRuleSubjectModel).where(
                ...
                AdmissionBenefitRuleSubjectModel.rule_source_snapshot_hash == value.provenance.source_snapshot_hash,
            )
        )
    ...
    def _stale_snapshot_sources(self, snapshot: AdmissionBenefitsSnapshot, university_id: UniversityId | None) -> int:
        count = 0
    ...
    @staticmethod
    def _validate_source_run(snapshot: AdmissionBenefitsSnapshot, source_run_id: IngestRunId) -> None:
        for source in snapshot.sources:
            if source.run_id != source_run_id:
                raise ValueError("admission benefit source run does not match ingestion run")
        for provenance in _all_provenance(snapshot):
            if provenance.source_run_id != source_run_id:
                raise ValueError("admission benefit provenance run does not match ingestion run")
...
def _apply_values(row: Any, session: Session, model: type[Any], values: dict[str, object], identity: object) -> str:
    if row is None:
        session.add(model(**values))
        return "inserted"
    changed = any(getattr(row, key) != value for key, value in values.items() if key != "created_at")
    if changed:
...
def _evidence_values(provenance: Any, policy_version: Any | None = None) -> dict[str, object]:
    source = provenance.source
    schema_version = policy_version.schema_version if policy_version is not None else ADMISSION_BENEFITS_SCHEMA_VERSION
    parser_version = policy_version.parser_version if policy_version is not None else ADMISSION_BENEFITS_PARSER_VERSION
    rule_policy_version = policy_version.policy_version if policy_version is not None else ADMISSION_BENEFITS_POLICY_VERSION
    return {
        "source_kind": source.kind.value,
        "source_url": str(source.url),
...
def _to_policy(row: IndividualAchievementPolicyModel, rules: Iterable[IndividualAchievementRuleModel]) -> IndividualAchievementPolicy:
    return IndividualAchievementPolicy(
        university_id=row.university_id,
        admission_year=row.admission_year,
        education_level=_education_level(row.education_level),
        global_max_points=row.global_max_points,
        default_combination_policy=AchievementCombinationPolicy(row.default_combination_policy),
...
def _to_achievement_rule(row: IndividualAchievementRuleModel) -> IndividualAchievementRule:
...
def _deduplicate_sources(rows: Iterable[Any]) -> tuple[SourceAttribution, ...]:
    ...
    for row in rows:
        ...
        values[(attribution.kind.value, str(attribution.url), attribution.content_sha256)] = attribution
    return tuple(values[key] for key in sorted(values))


def _coverage_from_rows(rows: Iterable[Any], sources: tuple[SourceAttribution, ...]) -> Any:
    from andromeda.modules.admission_benefits.contracts.coverage import (
        AdmissionBenefitCoverage,
    )

    rows_tuple = tuple(rows)
    status = AdmissionBenefitCoverageStatus.COMPLETE if rows_tuple else AdmissionBenefitCoverageStatus.PARTIAL
    return AdmissionBenefitCoverage(
        ...
        conflicts=sum(getattr(row, "status", None) == RuleDataStatus.CONFLICT.value for row in rows_tuple),
...
def _profile_identity(row: AdmissionBenefitOlympiadProfileModel) -> tuple[str, int, str]:
    return row.id, row.admission_year, row.source_snapshot_hash
