...
class _BenefitEvidenceMixin:
    ...
    source_snapshot_hash: Mapped[str] = mapped_column(
        String(64), ForeignKey("source_snapshots.content_sha256"), nullable=False, primary_key=True
    )
    source_run_id: Mapped[str] = mapped_column(String(64), ForeignKey("ingest_runs.id"), nullable=False)
...
class AdmissionBenefitOlympiadModel(_BenefitEvidenceMixin, Base):
    ...
    __table_args__ = (
        CheckConstraint("admission_year >= 2000 AND admission_year <= 2100", name="ck_benefit_olympiad_year"),
        CheckConstraint("rsosh_level IS NULL OR rsosh_level BETWEEN 1 AND 3", name="ck_benefit_olympiad_rsosh_level"),
        CheckConstraint("status IN ('active', 'stale', 'review_required', 'conflict', 'unresolved')", name="ck_benefit_olympiad_status"),
...
class AdmissionBenefitOlympiadProfileModel(_BenefitEvidenceMixin, Base):
    ...
    olympiad_source_snapshot_hash: Mapped[str] = mapped_column(String(64), nullable=False)
    admission_year: Mapped[int] = mapped_column(Integer, primary_key=True)
    ...
    __table_args__ = (
        ...
        UniqueConstraint("olympiad_id", "admission_year", "profile_name", "source_snapshot_hash", name="uq_benefit_profile_name"),
        CheckConstraint("admission_year >= 2000 AND admission_year <= 2100", name="ck_benefit_profile_year"),
        CheckConstraint("status IN ('active', 'stale', 'review_required', 'conflict', 'unresolved')", name="ck_benefit_profile_status"),
        Index("ix_benefit_profiles_olympiad_year", "olympiad_id", "admission_year", "status"),
    )
...
class AdmissionBenefitProfileSubjectModel(Base):
    ...
    admission_year: Mapped[int] = mapped_column(Integer, primary_key=True)
    profile_source_snapshot_hash: Mapped[str] = mapped_column(String(64), primary_key=True)
...
class AdmissionBenefitRuleModel(_BenefitEvidenceMixin, Base):
    ...
    id: Mapped[str] = mapped_column(String(160), primary_key=True)
    admission_year: Mapped[int] = mapped_column(Integer, primary_key=True)
    university_id: Mapped[str] = mapped_column(String(64), ForeignKey("universities.id"), nullable=False)
    ...
    conditions_json: Mapped[list[object]] = mapped_column(JSON, nullable=False, default=list, server_default="[]")
    ...
    __table_args__ = (
        CheckConstraint("admission_year >= 2000 AND admission_year <= 2100", name="ck_benefit_rule_year"),
        CheckConstraint("route IN ('olympiad', 'vosh', 'international', 'special_right', 'preferential_right', 'special_quota', 'separate_quota', 'targeted', 'other')", name="ck_benefit_rule_route"),
        CheckConstraint("scope_mode IN ('all', 'only', 'all_except')", name="ck_benefit_rule_scope_mode"),
        CheckConstraint("confirmation_requirement IN ('required', 'not_required', 'unknown')", name="ck_benefit_rule_confirmation"),
        CheckConstraint("result_type IS NULL OR result_type IN ('winner', 'prize_winner', 'team_member')", name="ck_benefit_rule_result_type"),
        CheckConstraint("points IS NULL OR (points >= 0 AND points <= 100)", name="ck_benefit_rule_points"),
        CheckConstraint("max_age_years IS NULL OR max_age_years >= 0", name="ck_benefit_rule_max_age"),
        CheckConstraint("status IN ('active', 'stale', 'review_required', 'conflict', 'unresolved')", name="ck_benefit_rule_status"),
        Index("ix_benefit_rules_university_year_active", "university_id", "admission_year", "education_level", "benefit_type", "status"),
        Index("ix_benefit_rules_olympiad_result", "olympiad_id", "olympiad_profile_id", "result_type", "admission_year", "status"),
        Index("ix_benefit_rules_source_refresh", "source_snapshot_hash", "source_run_id", "status"),
...
class AdmissionBenefitRuleScopeModel(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("target_kind IN ('direction', 'program', 'nps', 'education_level')", name="ck_benefit_scope_target_kind"),
        CheckConstraint("resolution IN ('resolved', 'unresolved')", name="ck_benefit_scope_resolution"),
        Index("ix_benefit_scope_target_lookup", "target_kind", "target_value", "admission_year", "excluded"),
    )
...
class AdmissionBenefitRuleSubjectModel(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("minimum_score IS NULL OR (minimum_score >= 0 AND minimum_score <= 100)", name="ck_benefit_rule_subject_minimum"),
        CheckConstraint("exam_kind IN ('ege', 'internal_exam', 'other', 'unknown')", name="ck_benefit_rule_subject_exam_kind"),
...
class IndividualAchievementPolicyModel(_BenefitEvidenceMixin, Base):
    ...
    global_max_points: Mapped[Decimal | None] = mapped_column(Numeric(5, 2), nullable=True)
    ...
    __table_args__ = (
        UniqueConstraint("university_id", "admission_year", "education_level", "source_snapshot_hash", name="uq_individual_achievement_policy_scope"),
        CheckConstraint("admission_year >= 2000 AND admission_year <= 2100", name="ck_achievement_policy_year"),
        CheckConstraint("global_max_points IS NULL OR (global_max_points >= 0 AND global_max_points <= 100)", name="ck_achievement_policy_global_max"),
        CheckConstraint("default_combination_policy IN ('additive', 'max_only', 'mutually_exclusive', 'not_combinable', 'unknown')", name="ck_achievement_policy_combination"),
        CheckConstraint("status IN ('active', 'stale', 'review_required', 'conflict', 'unresolved')", name="ck_achievement_policy_status"),
        Index("ix_achievement_policies_university_year", "university_id", "admission_year", "education_level", "status"),
    )
...
class IndividualAchievementRuleModel(_BenefitEvidenceMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("admission_year >= 2000 AND admission_year <= 2100", name="ck_achievement_rule_year"),
        CheckConstraint("points >= 0 AND points <= 100", name="ck_achievement_rule_points"),
        CheckConstraint("category_cap IS NULL OR (category_cap >= 0 AND category_cap <= 100)", name="ck_achievement_rule_category_cap"),
        CheckConstraint("combination_policy IN ('additive', 'max_only', 'mutually_exclusive', 'not_combinable', 'unknown')", name="ck_achievement_rule_combination"),
        CheckConstraint("status IN ('active', 'stale', 'review_required', 'conflict', 'unresolved')", name="ck_achievement_rule_status"),
        Index("ix_achievement_rules_lookup", "university_id", "admission_year", "education_level", "achievement_code", "status"),
        Index("ix_achievement_rules_source_refresh", "source_snapshot_hash", "source_run_id", "status"),
        Index("ix_achievement_rules_combination", "combination_group", "combination_policy", "admission_year"),
    )
