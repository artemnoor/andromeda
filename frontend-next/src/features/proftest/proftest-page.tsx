...
import { ApiRequestError, completeProftestSession, getCurrentProftestSession, nextProftestSession, saveProftestSession, startProftestSession } from "@/lib/api";
...
export function ProftestPage({ navigate }: { navigate: (route: Route) => void }) {
  ...
  useEffect(() => {
    ...
    getCurrentProftestSession()
      ...
      .catch((reason: unknown) => {
        if (!active) return;
        if (reason instanceof ApiRequestError && reason.status === 404) setPhase("intro");
  ...
  const advance = async () => {
    ...
    try {
      ...
      setError(reason instanceof ApiRequestError && reason.status === 409 ? "Сессия изменилась в другой вкладке. Обновите страницу и продолжите с последнего сохранённого шага." : "Не удалось сохранить ответ.");
    } finally { setBusy(false); }
  };
  ...
  return (
    <div className="mx-auto w-full max-w-3xl">
      ...
      <div className="mt-6 flex items-center justify-between gap-3">
        ...
        <Button onClick={() => void advance()} disabled={busy || (current.required && (answers[current.id]?.optionIds ?? []).length === 0 && !["uncertain", "skipped"].includes(answers[current.id]?.status ?? "answered"))} className="gap-2 bg-primary text-primary-foreground hover:bg-primary/90">
